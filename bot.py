...
def checkout_handler(message):
    ...
    reply_markup.add(users[chat_id]["phone_number"]
                     ) if users[chat_id]["phone_number"] else None
    reply_markup.add("Main Menu")
    bot.send_message(chat_id=message.chat.id, text=text,
                     reply_markup=reply_markup)
...
def name_handler(message):
    ...
    text = "will be delivered to\n\n{}\n\n{}\n\n{}\n\nPlease confirm your order".format(
        users[chat_id]["name"], users[chat_id]["address"], users[chat_id]["phone_number"])
    bot.send_message(chat_id=message.chat.id, text=text)
...
bot.set_webhook("http://127.0.0.1:5000/")
